...
async function homepage(request, response) {
	...
	const todoItems = await db.db.collection('todos').find({ deletedAt: { $exists : false }}).toArray()
	...
	const itemsLeft = await db.db.collection('todos').countDocuments({completed: false, deletedAt: { $exists : false }})
